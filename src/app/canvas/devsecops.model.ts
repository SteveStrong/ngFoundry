...
const core = DevSecOpsShapes.mixin('core', {
  color: 'blue',
  opacity: 0.5,
  width: 50,
  height: 50
...
DevSecOpsShapes.define('body', foPath2D, {
  path:
    // tslint:disable-next-line:max-line-length
    'M13.38,201.93a101.41,101.41,0,0,1,2-19.62,99.45,99.45,0,0,1,27-50.85,96.1,96.1,0,0,1,21.5-16.3c1.32-.74,2-.19,2.86.56a64,64,0,0,0,82.22.46l2-1.62c34.2,17.57,52.4,55.17,51.68,87.36Z'
}).mixin(core);

DevSecOpsShapes.define('head', foPath2D, {
  path:
    'M55.62,66.49c-.22-28.7,23.73-52.88,53.65-52.41,28,.44,51.48,24.15,51.41,52.53a52.53,52.53,0,1,1-105.06-.13Z'
}).mixin(core);
...
DevSecOpsShapes.define<shapeDevOps>('UI', shapeUI, {}).mixin(core);

DevSecOpsShapes.define<shapeDevOps>('Service', shapeService, {}).mixin(core);

DevSecOpsShapes.define<shapeDevOps>('Data', shapeData, {}).mixin(core);
...
DevSecOpsShapes.define<shapeDevOps>('App', shapeApp, {
  ...
  const UI = DevSecOpsShapes.find('UI')
    .makeComponent(obj)
    .setName('UI');
  const Service = DevSecOpsShapes.find('Service')
    .makeComponent(obj)
    .setName('Service');
  const Data = DevSecOpsShapes.find('Data')
    .makeComponent(obj)
    .setName('Data');
  const InvEnv = DevSecOpsShapes.find('InvEnv')
    .makeComponent(obj)
    .setName('InvEnv');
  ...
  constructor(properties?: any) {
    ...
    this.override({
      height: function() {
        ...
        const size = parseInt(this.size.split(':')[1]);
        return 25 * size;
      },
      width: function() {
        // tslint:disable-next-line:radix
        const size = parseInt(this.size.split(':')[0]);
        return 25 * size;
      }
    });
  }
