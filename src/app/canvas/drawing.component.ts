...
export class DrawingComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('world') public worldRef: ElementRef;
  ...
  doSubShape(page: foPage) {
    const result = ShapeStencil.factories.getItem('doAddSubGlyph').run();
    result.forEach(item => {
      item.addAsSubcomponent(page);
    });
  }

  initLifecycle() {
    //Lifecycle.mute = true;
    const Lifecycle2D = Lifecycle.observable.pipe(filter(e => e.object.is2D()));

    // Lifecycle2D.subscribe(event => {
    //   console.log(event.id, event.cmd, event.myGuid, JSON.stringify(event.value));
    // });

    const glued = Lifecycle2D.pipe(filter(e => e.isCmd('glued')));
    const moved = Lifecycle2D.pipe(filter(e => e.isCmd('moved')));
    const dropped = Lifecycle2D.pipe(filter(e => e.isCmd('dropped')));
    const reparent = Lifecycle2D.pipe(filter(e => e.isCmd('reparent')));
    const created = Lifecycle2D.pipe(filter(e => e.isCmd('created') && e.value));
    const changed = Lifecycle2D.pipe(filter(e => e.isCmd('changed') && e.value));

    glued.subscribe(event => {
      const glue = event.object as foGlue2D;
      const { sourceGuid, sourceName, targetGuid, targetName } = glue.signature;
    ...
    reparent.subscribe(event => {
      ...
      this.currentStudio.currentStage.found(event.myGuid, shape => {
        const parent = event.object.myParent();
        parent &&
          this.currentStudio.currentStage.found(
            parent.myGuid,
            item => {
              shape.reParent(item);
            },
            miss => {
              shape.reParent(this.currentStudio.currentStage);
            }
          );
      });
    });
    ...
    created.subscribe(event => {
      const stage = this.currentStudio.currentStage;
      stage.findItem(event.myGuid, () => {
        const knowledge = event.value;

        const myClass = event.myClass.replace('2D::', '3D::');
        const concept = globalWorkspace
          .select(item => Tools.matches(item.myName, myClass))
          .first();
        if (concept) {
          const result = concept.newInstance(event.object.asJson);
          stage.establishInDictionary(result);
        } else {
          knowledge &&
            knowledge.usingRuntimeType('foGlyph3D', know => {
              const result = know.newInstance(event.object.asJson);
              stage.establishInDictionary(result);
            });
        }
      });
  ...
  ngOnInit() {
    ...
    const libs = this.rootWorkspace.stencil;
  ...
  public ngAfterViewInit() {
    ...
    this.screen3D.setRoot(
      this.worldRef.nativeElement,
      this.pageWidth,
      this.pageHeight
    );
