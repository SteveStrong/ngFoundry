...
export class foSolution extends foKnowledge {
    ...
    private addAltStructureSpec(structure: foStructure, exist: WhereClause<foComponent>, priority?: number): foAltStructureSpec {
        if (!this._structures) {
            this._structures = new foDictionary<foAltStructureSpec>();
        }
        let altSpec = new foAltStructureSpec({
            exist,
            structure,
            priority: priority
        });
    ...
    private addSubStructureSpec(name: string, solution: foSolution, properties?:any): foSubSolutionSpec {
        if (!this._solutions) {
            this._solutions = new foDictionary<foSubSolutionSpec>();
        }
        let subSpec = new foSubSolutionSpec({
            name,
            solution,
    ...
    subSolution(name: string, spec?: any | foSolution, properties?:any) {
        let structure = spec instanceof foSolution ? spec : new foSolution(spec, this);
    ...
    makeComponent(parent?: any, properties?: any, onComplete?: Action<any>): any {
        ...
        let result;
        this.altStructures && this.altStructures.forEach(item => {
        ...
        result && this.solutions && this.solutions.forEach(item => {
