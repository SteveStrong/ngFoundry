...
(function (ns, undefined) {
	...
	function onCurrentSmash(smashFn) {
	    var current = fo.currentComputingProperty && fo.currentComputingProperty();
	    if (!current) return;
	    current.onValueSmash = smashFn;
	}
	ns.onCurrentSmash = onCurrentSmash;


    //return a function the 
	function getDefault(fallback, name) {
        var fallbackFn = ns.tools.isFunction(fallback) ? fallback: function () { return fallback; }

	    function searchForDefault() {
	        var parent = this.myParent;

	        if (!parent) return fallbackFn.call(this);

	        var current = fo.currentComputingProperty && fo.currentComputingProperty();
	        var searchName = name ? name : current && current.myName;
	        if (!searchName) return fallbackFn.call(this);

	        var result;
	        try {
	            if (parent && parent.resolveSuperior) {
	                result = parent.resolveSuperior(searchName);
	            }
	            else if (parent && parent.hasOwnProperty(searchName)) {
	                result = parent[searchName];
	                return result;
	            }
	        } catch (e) {
	        ...
	        return result || fallbackFn.call(this);
	    }
	    return searchForDefault;
	}
	ns.getDefault = getDefault;
	...
	ns.tools = {
        ...
        cleanFormulaText: function (formula) {
            ...
            return ns.tools.removeExtraSpaces(ns.tools.removeCRLF(ns.tools.removeJSComments(text)));
        },
        cleanTemplateHtml: function (html) {
            var text = html;
            return ns.tools.removeCRLF(html);
        },
		...
		computeNamespace: function(space, obj){
		    return '{0}::{1}'.format(space, ns.tools.getType(obj));
		},
		getHttpContext: function () {
		    if (!location) return; //may not be running in browser;
		...
		mixExact: function (target, source) {
		    if (!source) return target;
		    if (!target) return source;
		    for (var key in source) {
		...
		intersect: function (target, source) {
		    if (!source) return target;
		    if (!target) return source;
		    var intersect = {};
		    for (var key in target) {
	    ...
	    //template("Hello {who}!", { who: "JavaScript" });  
        // "Hello JavaScript!" 
	    ...
	    /**
	     * this is used to convert a an object of methods and values into all computed values
	     */
	    asSpecObject: function (spec) {
	        if (!spec) return;
	        var result = {};
	        for (var key in spec) {
	            var value = spec[key];
	            if (ns.tools.isFunction(value)) {
	                ns.tools.defineCalculatedProperty(result, key, value);
	            } else {
	                result[key] = value;
	            }
	        }
