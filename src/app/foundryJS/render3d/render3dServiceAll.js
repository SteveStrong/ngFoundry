...
var lib3D = (function (fo, tools, three, undefined) {

    function lightPrimitive(type, data) {
        var light;
        ...
        switch (type) {
            case 'point':
                light = new THREE.PointLight(
                    data.color || 0xffffbb,  // — Numeric value of the RGB color.
                    data.intensity || 1,       // — Numeric value of the light's strength/intensity. 
                    data.distance || 100,      // -- The distance of the light where the intensity is 0. When distance is 0, then the distance is endless. 
                    data.decay || 1           // -- The amount the light dims along the distance of the light. 
                );
                break;
            case 'spot':
                light = new THREE.PointLight(
                    ...
                    data.intensity || 1,       // — Numeric value of the light's strength/intensity. 
                    data.distance || 100,      // -- The distance of the light where the intensity is 0. When distance is 0, then the distance is endless. 
                    data.angle || Math.PI/3.,      // -- Maximum angle of light dispersion from its direction whose upper bound is Math.PI/2.
                    data.exponent || 10,      // -- Rapidity of the falloff of light from its target direction.
                    data.decay || 1           // -- The amount the light dims along the distance of the light. 
                );
                break;
            case 'hemisphere':
                light = new THREE.HemisphereLight(
                    data.skyColor || 0xffffbb,      // — Numeric value of the RGB sky color.
                    data.groundColor || 0x080820,   // — Numeric value of the RGB ground color.
                    data.intensity || 1             // — Numeric value of the light's strength/intensity. 
                );
                break;
            case 'directional':
                light = new THREE.DirectionalLight(
                    data.color || 0xffffff,     // soft white light
                    data.intensity || 1         // — Numeric value of the light's strength/intensity. 
                );
                break;
            case 'ambient':
    ...
    function cameraPrimitive(type, data) {
            type = data && data.type ? data.type : type;
    ...
    var defaultCameras = {
        perspective: cameraPrimitive('perspective', {
            type: 'perspective',
            fov: 75,
        ...
        cube: cameraPrimitive('cube', {
            type: 'cube',
        }),
        orthographic: cameraPrimitive('orthographic', {
            type: 'orthographic',
            left: 15,
            right: -15,
            top: 15,
    ...
    function materialPrimitive(type, data) {
        var material;
        type = data && data.type ? data.type : type;
        switch (type) {
            case 'lambert':
    ...
    function geometryPrimitive (type, data, curve) {
        var geometry;
        type = data && data.type ? data.type : type;
        switch (type) {
            case 'block':
    ...
    function splinePrimitive(type, data) {
        var curve;
        type = data && data.type ? data.type : type;
        switch (type) {
            case 'spline':
...
(function (app, fo, tools, engine3D, undefined) {
    ...
    function distanceTo(lat1, lon1, lat2, lon2, radius) {
    ...
    function destinationTo(lat1, lon1, brng, dist, radius) {
        var lat1 = lat1.toRad();
        var lon1 = lon1.toRad();
        radius = radius ? radius : EARTH_RADIUS;
    ...
    function destinationToVector3(lat1, lon1, brng, dist, radius) {
        var pos = destinationTo(lat1, lon1, brng, dist, radius)
        return latLongToVector3(pos.lat, pos.lon, radius)
    }


    app.service('renderDisplay3D', function ($q) {

        var self = this;
        self.latLongToVector3 = latLongToVector3;
        self.llToPosition = latLongToVector3;
        self.llToBearing = bearingTo;
        self.llToDistance = distanceTo;
        self.llToDestination = destinationTo;
        self.llToDestinationPosition = destinationToVector3;

        self.create = function (id) {
            return engine3D.create(id);
        }
        self.activate = function (id) {
            return engine3D.activate(id);
        }
        self.active = function () {
            return engine3D.active();
        }
        self.find = function (id) {
            return engine3D.find(id);
        }

        self.addGlobe = function (id, noTexture, radius) {
            var material;
            var display = (id instanceof engine3D.engine) ? id : self.find(id);
            if (!display || display.globeMesh) return;
            ...
            display.globeMesh = new THREE.Mesh(spGeo, material);
            display.add(display.globeMesh);

            return this;
        };

        self.removeGlobe = function (id) {
            var display = (id instanceof Core3DEngine) ? id : self.find(id);
            if (!display || !display.globeMesh) return;

            display.remove(display.globeMesh);
            display.globeMesh = undefined;
        };

        function definePrimitive(name, geometrySpec, materialSpec) {
            var deferred = $q.defer();

            var model = engine3D.definePrimitive(name, geometrySpec, materialSpec);
        ...
        function defineModel(name, file) {
            ...
            geo.jsonLoader.load(file, function (geometry, materials) {
                var model = engine3D.defineModel(name, geometry, materials);
                deferred.resolve(model);
            });
        ...
        self.isWebGLSupported = isWebGLSupported;


    });

    app.service('designModel3D', function (renderDisplay3D) {

        var self = this;

        self.create = function (id) {
            return renderDisplay3D.create(id);
        }
        self.activate = function (id) {
            return renderDisplay3D.activate(id);
        }
        self.active = function () {
            return renderDisplay3D.active();
        }
        self.find = function (id) {
            return renderDisplay3D.find(id);
        }
        ...
        var entity3D = fo.establishType('3d::entity', {
            ...
            model: function () {
                ...
                var model = Scene3D.definePrimitive(type, context, material);
                return model;
            },
            instance: function () {
                var context = this.context;
        ...
        var viewModelRoot = {};
        function findRootViewModel(obj) {
            var found = viewModelRoot[obj.myGuid];
            if (found) return found;
            var parent = obj.myParent;
            if (!parent) {
                throw new Error('findRootViewModel!');
            }
        ...
        function establishViewModel(context, display) {
            var viewModel = viewModelDictionary[context.myGuid];
            if (viewModel) {
                return viewModel;
            }

            viewModel = self.newInstance({
                context: context.evaluate().unique(),  //compute values outside of render loop
                myName: context.myName,
                myGuid: context.myGuid,
                display: display || self.active(),
                geom: function () {
                    var root = this.display;
                    return root.getRootGeom();
                },
            });
            viewModel.activate = function () {
                self.activate(this.display);
                return this;
            }
            viewModelDictionary[viewModel.myGuid] = viewModel;
            viewModelRoot[viewModel.myGuid] = viewModel;
            return viewModel;
        }
        ...
        function renderModelAs(context, isRoot) {
            if (context && isRoot) {
                return establishViewModel(context, self.active());
            }
        ...
        self.reRender = function (item) {
            var viewModel = findRootViewModel(item);
            //viewModel.render(false);
            viewModel.activate();

            //item.purgeMySubcomponents();
            //self.renderModelAs(item);

            item.render();
            return viewModel;
        }

        self.flushModel = function (model) {
            var viewModel = findRootViewModel(model);
            viewModel.render(false);
            model.purgeMySubcomponents();
            return viewModel;
        }
